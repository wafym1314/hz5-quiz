...
if (want('--e2e')) {
  ...
  // 这一组要逐个年级、逐个科目地在真实浏览器里翻章节页，是所有 e2e 里最慢的一组（约 8 分钟）。
  // 2026-09-11：它之前一直卡在 600s/900s 超时，真正原因不是跑得慢，而是断言跑完后
  // `browser.close()` 挂住不返回（上一组 e2e 残留的 chrome 进程导致）。已给四组 e2e 都加了
  // 「最多等 5 秒就走」的关闭保护；这里再留 900s 余量，兜住机器慢的情况。
