...
// 现改为自动挑全库题量最大的章节。题库将来扩容或收缩都不会再让这条断言失真；
// 若全库确实没有 >20 题的章节，明确跳过。
// 另外「新题 ≥10 道」这条阈值也要跟着章节实际余量走：第一次已经抽走 20 道，
// 本章剩下的新题最多只有 (总题数 − 20) 道，写死 10 会在章节只剩几道题时必然失败
// （2026-09-11 各章已普遍重建为 20~26 题，最大章 5sci-1 只有 26 题 → 余量仅 6 道）。
let big = null;
...
if (!big || big.n <= 20) {
  ...
  // 本章第一次抽走 20 道，剩下没练过的最多只有 big.n − 20 道，按余量自适应
  const wantNew = Math.min(10, big.n - 20);
  chk('第二次进入带来足量新题', m2new >= wantNew,
      '新题 ' + m2new + ' 道（本章 ' + big.n + ' 题，余量应 ≥' + wantNew + '）');
