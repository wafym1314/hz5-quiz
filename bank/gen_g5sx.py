# -*- coding: utf-8 -*-
"""五年级数学 · 北师版 题库生成器（2026-09-10 整套重写）

背景：旧 5sx 由 gen_math_bs5.py 生成，370 题里同一道题被复制多遍只打乱选项顺序，
      知识点重复率 78.4%；15 个单元全部没有填空题，6 个单元拔高题不足 4 道。
      本生成器按「每单元 20 个互不相同的知识点、每个知识点 1 题」重建：
      五上 7 单元 + 五下 8 单元 = 15 单元 × 20 题 = 300 题。
...
OUT = os.path.join(HERE, 'new', 'g5sx.js')
rnd = random.Random(20260910)

rows = []
_st = {'code': None, 'ch': None, 'i': 600000}
...
import _g5sx_a  # noqa: E402  五上 第1~2单元
import _g5sx_b  # noqa: E402  五上 第3~5单元
import _g5sx_c  # noqa: E402  五上 第6~7单元 + 五下 第1~2单元
import _g5sx_d  # noqa: E402  五下 第3~5单元
import _g5sx_e  # noqa: E402  五下 第6~8单元

for mod in (_g5sx_a, _g5sx_b, _g5sx_c, _g5sx_d, _g5sx_e):
    mod.build(api)

# ---------------- 自检 ----------------
assert len(rows) == 300, '题量应为 300，实际 %d' % len(rows)
units = {}
for r in rows:
    units.setdefault(r['c'], []).append(r)
assert len(units) == 15, '应为 15 个单元，实际 %d' % len(units)
